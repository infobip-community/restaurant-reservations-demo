...
import { MeetingsPropsI, RowPropsI } from "./Reservations.types";
import {MeetingI, ReservationI, TimeFrameI} from "../../app/App.types";
...
import {
  ...
  CardActions,
  CardContent,
  createFilterOptions, FormControl, Grid, InputLabel, MenuItem, Select, SelectChangeEvent,
  TextField,
  Typography
} from "@mui/material";
import {CalendarMonth, Dining, Diversity3, Groups2, LocalDining, QueryBuilder} from "@mui/icons-material";
...
const Reservations = ({
  ...
  const [temporalReservation, setTemporalReservation] = useState<ReservationI | undefined>();
  ...
  const selectedReservation = (event: React.SyntheticEvent, newValue: string | null) => {
    const reservation = reservationsList.find(reservation => reservation.host_name === newValue)
    setReservationSelected(reservation);
    setTemporalReservation(reservation);
    if(reservation){
      setDate(new Date(reservation.date ? reservation.date : ''));
      setStartTime(new Date( reservation.hour ? `${reservation.date} ${reservation.hour}` : ''));
    }
  }
  ...
  const handleSave = async () => {
    const promise = await fetch(`${APIPath}`, {
      method: "PUT",
      headers: { "Content-type": "application/json; charset=UTF-8" },
  ...
  const handleChangeInput = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement> | SelectChangeEvent) => {
    ...
    setTemporalReservation({...temporalReservation,  [e.target.name]: e.target.value})
  };
  ...
  return (
    <>
      ...
      {
        reservationSelected && <Card variant="outlined">
          <Box sx={{m: 2}}>
            <CardContent>
              <Restaurant/>
              ...
              <Box sx={{display: 'flex', flexFlow: 'row', pt: 2}}>
                {editing ? <FormControl fullWidth>
                  ...
                  <Select
                      name='party_size'
                      value={temporalReservation ? temporalReservation.party_size : reservationSelected.party_size}
                      label="size"
            ...
            <CardActions sx={{display: 'flex', flexFlow: 'row-reverse'}}>
              <Button size="small" onClick={editing ? handleCloseEdit : handleDelete} startIcon={<LocalDining/>}>{editing ? 'Cancel' : 'Delete'}</Button>
              <Button size="small" sx={{pr: 5}} onClick={editing ? handleSave : handleOpenEdit} startIcon={<Dining/>}>{editing ? 'Save' : 'Edit'}</Button>
            </CardActions>
          </Box>
        </Card>
      }
